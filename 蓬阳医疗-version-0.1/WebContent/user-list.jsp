...
<body>
<nav class="Hui-breadcrumb"><i class="icon-home"></i> 首页 <span class="c-gray en">&gt;</span>病例信息管理 <span class="c-gray en">&gt;</span> 病例管理 <a id="refresh" href="${requestScope.refresh}" class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" title="刷新" onfocus="alert（）"><i class="icon-refresh"></i></a></nav>
<div class="pd-20">
  <div class="text-c"> 
    <form action="/test/patient/findByNameOrNum.do?page=1" method="post">
    <input type="text" class="input-text" style="width:250px" placeholder="输入姓名或医疗号" id="mess" name="mess" oninput="$('#search-button').attr('disabled',false)"><button disabled="disabled" type="submit" class="btn btn-success" id="search-button" name=""><i class="icon-search"></i> 搜索</button>
	</form>      
  </div>
  ...
  <table style="width: 1680px;" align="center" class="table table-border table-bordered table-hover table-bg table-sort" width="815px">
    ...
    <c:if test="${empty paList}"><tr class="text-c" align="center"><p>还没有一条记录</p></tr></c:if>
    <c:if test="${ not empty paList}">
      <tr class="text-c">
       <!--  <td width="25px"><input type="checkbox" value="1" name=""></td> -->
        ...
        <td width="130px"><a href="baogao.do?fileAddress=${patient.pdf_path}">${patient.pdf_path}</a></td>
 ...
 <div style="width: 100%;height: 100px;"></div>
	...
	<input id="returnMess" type="hidden" value="${requestScope.mess}"/> 
